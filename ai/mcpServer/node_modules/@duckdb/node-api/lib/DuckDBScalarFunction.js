...
const DuckDBScalarFunctionBindInfo_1 = require("./DuckDBScalarFunctionBindInfo");
const DuckDBScalarFunctionInfo_1 = require("./DuckDBScalarFunctionInfo");
const DuckDBScalarFunctionInitInfo_1 = require("./DuckDBScalarFunctionInitInfo");
...
class DuckDBScalarFunction {
    ...
    static create({ name, bindFunction, initFunction, mainFunction, returnType, parameterTypes, varArgsType, specialHandling, volatile, extraInfo, }) {
    ...
    setBindFunction(bindFunction) {
        node_bindings_1.default.scalar_function_set_bind(this.scalar_function, (info) => {
            const bindInfo = new DuckDBScalarFunctionBindInfo_1.DuckDBScalarFunctionBindInfo(info);
            bindFunction(bindInfo);
        });
    }
    setInitFunction(initFunction) {
        node_bindings_1.default.scalar_function_set_init(this.scalar_function, (info) => {
            initFunction(new DuckDBScalarFunctionInitInfo_1.DuckDBScalarFunctionInitInfo(info));
        });
    }
    setMainFunction(mainFunction) {
        node_bindings_1.default.scalar_function_set_function(this.scalar_function, (info, input, output) => {
            const functionInfo = new DuckDBScalarFunctionInfo_1.DuckDBScalarFunctionInfo(info);
