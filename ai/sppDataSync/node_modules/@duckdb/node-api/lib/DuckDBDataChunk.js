...
class DuckDBDataChunk {
    ...
    static create(types, rowCount) {
        let chunk;
        try {
            chunk = new DuckDBDataChunk(node_bindings_1.default.create_data_chunk(types.map((t) => t.toLogicalType().logical_type)));
        }
        catch (cause) {
            ...
            throw new Error(`Cannot create a data chunk with these column types: ${types.join(', ')}`, { cause });
        }
    ...
    assertColumnCount(count) {
        ...
        if (count !== columnCount) {
            throw new Error(`Provided number of columns (${count}) does not match chunk column count (${columnCount})`);
        }
    }
     ...
     */
    setColumnValuesConverted(columnIndex, values, converter) {
        const vector = this.getColumnVector(columnIndex);
        if (vector.itemCount !== values.length) {
            throw new Error(`number of values must equal chunk row count`);
        }
        ...
        try {
            for (let i = 0; i < values.length; i++) {
                vector.setItem(i, converter(values[i], type, converter));
            }
        }
        catch (cause) {
     ...
     */
    setColumnsConverted(columns, converter) {
        this.assertColumnCount(columns.length);
        if (columns.length > 0) {
            this.rowCount = columns[0].length;
        }
        for (let columnIndex = 0; columnIndex < columns.length; columnIndex++) {
            this.setColumnValuesConverted(columnIndex, columns[columnIndex], converter);
        }
    }
     ...
     * thing the caller supplies.
     */
    setRowsConverted(rows, converter) {
        this.rowCount = rows.length;
        const columnCount = this.columnCount;
        for (let columnIndex = 0; columnIndex < columnCount; columnIndex++) {
            const vector = this.getColumnVector(columnIndex);
            ...
            try {
                for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
                    vector.setItem(rowIndex, converter(rows[rowIndex][columnIndex], type, converter));
                }
            }
            catch (cause) {
                throw new Error(`Failed to set column ${columnIndex} (${type})`, {
