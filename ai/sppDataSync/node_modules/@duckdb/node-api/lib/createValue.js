    ...
    switch (typeId) {
        ...
        case DuckDBTypeId_1.DuckDBTypeId.BIGINT:
            if (typeof input === 'bigint' || typeof input === 'number') {
                return node_bindings_1.default.create_int64(BigInt(input));
            }
        ...
        case DuckDBTypeId_1.DuckDBTypeId.UBIGINT:
            if (typeof input === 'bigint' || typeof input === 'number') {
                return node_bindings_1.default.create_uint64(BigInt(input));
            }
            throw new Error(`input is not a bigint`);
        ...
        case DuckDBTypeId_1.DuckDBTypeId.STRUCT:
            if (input instanceof values_1.DuckDBStructValue) {
                ...
                return node_bindings_1.default.create_struct_value(type.toLogicalType().logical_type, type.entryNames.map((name, i) => {
                    if (!(name in input.entries)) {
                        throw new Error(`STRUCT entry '${name}' is missing from input`);
                    }
        ...
        case DuckDBTypeId_1.DuckDBTypeId.GEOMETRY:
            if (input instanceof values_1.DuckDBGeometryValue) {
        ...
        case DuckDBTypeId_1.DuckDBTypeId.VARIANT: {
            ...
            const inner = input instanceof values_1.DuckDBVariantValue ? input.value : input;
            const innerType = input instanceof values_1.DuckDBVariantValue && input.type
                ? input.type
                : (0, typeForValue_1.typeForValue)(inner);
            try {
                return createValue(innerType, inner);
            }
            catch (err) {
                throw new Error(`Failed to create VARIANT value (inner type ${innerType}): ${err.message}`);
            }
        }
        default:
