...
export const handler = async (event) => {
  try {
    ...
    const keys =
      ...
      (body.integrationKey ? [body.integrationKey] : null);
    if (!keys?.length) {
      return response(400, {
        error: "Provide integrationKey or integrationKeys[]",
      });
    }
    ...
    const results = await Promise.allSettled(
      keys.map(async (integrationKey) => {
        ...
        console.log(
          `[refreshAccessToken] Refreshed tokens for "${integrationKey}", expires_at=${saved.expires_at}`,
        );
        return {
          integrationKey,
          success: true,
